...
async function main() {
    ...
    // Date de mise en ligne : derniers 7 jours
    const today = new Date();
    const weekAgo = new Date(today.getTime() - 7 * 86_400_000);

    // NOTE: les noms exacts des champs (id/name) doivent être vérifiés dans le
    // DOM réel — à ajuster une fois testé. Exemple probable basé sur PRADO :
    ...
    const MAX_PAGES = 3; // sécurité, un peu au-dessus de 171 pages attendues
    while (hasNextPage) {
        ...
        // Pagination : bouton "suivant" — à ajuster selon le vrai sélecteur
    ...
    // console.log(`Extraction page ${pageNum}... (${allResults.length} AO jusqu'ici)`);
